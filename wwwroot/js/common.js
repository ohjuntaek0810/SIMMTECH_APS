    ...
    */
    setExcelToGridPaste: (grid_id) => {
        ...
        function handlePaste(gridInstance, text) {
            ...
            const allVisibleColumns = gridInstance.getVisibleColumns(null, true); // 고정 컬럼 포함 전체
            ...
            // editable 컬럼만 필터링 (전체 인덱스 기준)
            const editableColumns = allVisibleColumns
                .map((col, index) => ({ ...col, _index: index }))
                .filter(col => col.allowEditing !== false && col.dataField);

            // focusedColumn 기준으로 editable 시작 인덱스 계산
            const startEditableIndex = editableColumns.findIndex(col => col._index >= focusedColumnIndex);
                ...
                rowData.forEach((cellData, dataOffset) => {
                    ...
                    if (targetCol) {
                        ...
                        if (targetCol.dataType === "number") {
                            value = parseFloat(cellData.replace(/[^\d.-]/g, ''));
                        }

                        // lookup 컬럼이면 displayExpr → valueExpr 변환
                        if (targetCol.lookup?.dataSource) {
                            const match = targetCol.lookup.dataSource.find(item =>
                                item[targetCol.lookup.displayExpr] === value
                            );
                            value = match ? match[targetCol.lookup.valueExpr] : value;
                        }
